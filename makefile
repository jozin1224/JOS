...
# yes, i dont know makefile

all: iso

iso:
	mkdir -p iso_root
	nasm -f elf32 src/Kernel/kernel_entry.asm -o Bin/kernel_entry.o
	g++ -m32 -ffreestanding -fno-pie -fno-exceptions -fno-rtti -c src/Kernel/kernel.cpp -o Bin/kernel.o
	ld -m elf_i386 -T linker.ld Bin/kernel_entry.o Bin/kernel.o -o Bin/kernel.bin
	nasm -fbin src/Bootloader/boot.asm -o Bin/boot.bin
